...
public class Ball implements Runnable {
	...
	Ball() {
		...
		r = 280;     
		 // 속이 빈 원의 중심 x좌표 = 375 + 265 = 640 , 속이 빈 원의 반지름은 265
		Ox = 640;  
		 // 속이 빈 원의 중심 y좌표 = 100 + 265 = 640 , 속이 빈 원의 반지름은 265
		Oy = 365;  
    ...
    /** Thread를 실행시키면 run함수가 실행
     *  따라서, run함수 안의  while문을 통해 Ball이 회전하고자 하는 원을 계속 회전하게 된다. */
	public void run() {
		// 곡이 끝날 때 까지 돌려줘야 하므로 계속 반복
		while (true) {
			try {

				// Ball이 속이 빈 원을 중심으로 돌아야 하므로 원 중심값과 원의 반지름을 더한 값인 Ox,Oy를 더해주어야 한다. 
